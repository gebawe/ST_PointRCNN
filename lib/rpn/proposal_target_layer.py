...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict):
        roi_boxes3d, gt_boxes3d, rcnn_weights = input_dict['roi_boxes3d'], input_dict['gt_boxes3d'], input_dict['rcnn_weights']
        

        batch_rois, batch_gt_of_rois, batch_roi_iou, batch_rcnn_weights = self.sample_rois_for_rcnn(roi_boxes3d, gt_boxes3d, rcnn_weights)
        ...
        if cfg.RCNN.USE_DEPTH:
            ...
            # lidar span of -100, 100 in x,y
            pts_depth = input_dict['pts_depth'] / np.max(cfg.PC_AREA_SCOPE) - 0.5
        ...
        output_dict = {'sampled_pts': sampled_pts.view(-1, cfg.RCNN.NUM_POINTS, 3),
                       ...
                       'rcnn_weights':batch_rcnn_weights.view(-1)}

        return output_dict

    def sample_rois_for_rcnn(self, roi_boxes3d, gt_boxes3d, rcnn_weights):
        """
        :param roi_boxes3d: (B, M, 7)
        :param gt_boxes3d: (B, N, 8) [x, y, z, h, w, l, ry, cls]
        :param rcnn_weights: confidence based weights based on the label type gt:1, pseduo:0.8-1.0
        :return
            batch_rois: (B, N, 7)
        ...
        ### batch rcnn weights 
        batch_roi_rcnn_weights = rcnn_weights.new(batch_size, cfg.RCNN.ROI_PER_IMAGE).zero_()
 

        for idx in range(batch_size):
            cur_roi, cur_gt, cur_weights = roi_boxes3d[idx], gt_boxes3d[idx], rcnn_weights[idx]

            k = cur_gt.__len__() - 1
            ###while cur_gt[k].sum() == 0:
            ### ***************************** bug fix ******************************************************************
            while (cur_gt[k].sum() == 0) & (abs(k) < cur_gt.__len__()):            
                k -= 1
            cur_gt = cur_gt[:k + 1]
            ...
            roi_list, roi_iou_list, roi_gt_list, roi_rcnn_weights = [], [], [], []
            if fg_rois_per_this_image > 0:
                fg_rois_src = cur_roi[fg_inds]
                gt_of_fg_rois = cur_gt[gt_assignment[fg_inds]]

                ###hack  roi weights 
                gt_roi_fg_rcnn_weights = cur_weights[gt_assignment[fg_inds]]
                ...
                #hack
                roi_rcnn_weights.append(gt_roi_fg_rcnn_weights)
            ...
            if bg_rois_per_this_image > 0:
                ...
                ###hack  roi weights 
                gt_roi_bg_rcnn_weights = cur_weights[gt_assignment[bg_inds]]
                ...
                ###hack
                roi_rcnn_weights.append(gt_roi_bg_rcnn_weights)

            rois = torch.cat(roi_list, dim=0)
            iou_of_rois = torch.cat(roi_iou_list, dim=0)
            gt_of_rois = torch.cat(roi_gt_list, dim=0)
            #hack
            rcnn_weights_of_rois = torch.cat(roi_rcnn_weights, dim=0)

            batch_rois[idx] = rois
            batch_gt_of_rois[idx] = gt_of_rois
            batch_roi_iou[idx] = iou_of_rois
            #hack
            batch_roi_rcnn_weights[idx] = rcnn_weights_of_rois

        return batch_rois, batch_gt_of_rois, batch_roi_iou, batch_roi_rcnn_weights
