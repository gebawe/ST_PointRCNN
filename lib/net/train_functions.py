...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        ...
        if cfg.RPN.ENABLED:
            ...
            # data has sample_id keyword, which should say name of file

            if not cfg.RPN.FIXED:
                rpn_cls_label, rpn_reg_label = data['rpn_cls_label'], data['rpn_reg_label']
                
                #rpn weights 
                rpn_weights = data['rpn_weights']
                ...
                #rpn weights 
                rpn_weights = torch.from_numpy(rpn_weights).cuda(non_blocking=True).float()
            ...
            if 'confidence' in data.keys():
                rcnn_weights = torch.from_numpy(data['confidence']).cuda(non_blocking=True).float()
                rcnn_weights = torch.squeeze(rcnn_weights)
                input_data = {'pts_input': inputs, 'gt_boxes3d': gt_boxes3d, 'rcnn_weights': rcnn_weights}
        ...
        if cfg.RPN.ENABLED and not cfg.RPN.FIXED:
            ...
            rpn_loss = get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict, rpn_weights)
    ...
    def get_rpn_loss(model, rpn_cls, rpn_reg, rpn_cls_label, rpn_reg_label, tb_dict, rpn_weights):
        ...
        rpn_weights_flat = rpn_weights.view(-1)
        ...
        if cfg.RPN.LOSS_CLS == 'DiceLoss':
            ...
            #with rpn weights
            rpn_loss_cls = rpn_cls_loss_func(rpn_cls_flat, rpn_cls_target, cls_weights, rpn_weights_flat)
        ...
        # RPN regression loss with weights
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        ...
        ###hack rcnn weights
        rcnn_weights = ret_dict['rcnn_weights']
        ...
        #hack rcnn weightes 
        rcnn_weights_flat = rcnn_weights.view(-1)
        ...
        if cfg.RCNN.LOSS_CLS == 'SigmoidFocalLoss':
            ...
            rcnn_loss_cls = (batch_loss_cls * cls_valid_mask * rcnn_weights_flat ).sum() / torch.clamp(cls_valid_mask.sum(), min=1.0)
