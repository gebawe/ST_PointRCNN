...
class RPN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        :return:
        """
        pts_t_input = input_data['pts_input']
        ...
        # crate falg for reference frame
        pts_ref_flag = (pts_time == 0)
        ref_idxs = np.where(pts_ref_flag == 1)[0]


        # ref backbone pts (B, N, 3)   
        ref_backbone_xyz = backbone_xyz[:, ref_idxs, :]
        
        # ref backbone feature (B, C, N)    
        ref_backbone_features = backbone_features[:, :, ref_idxs]
        
        
        ### Assing the reference frame xyz points and features to the rpn class and regration
        backbone_xyz = ref_backbone_xyz 
        backbone_features = ref_backbone_features


        rpn_cls = self.rpn_cls_layer(backbone_features).transpose(1, 2).contiguous()  # (B, Nt, 1)
        rpn_reg = self.rpn_reg_layer(backbone_features).transpose(1, 2).contiguous()  # (B, Nt, C)
