...
def get_objects_from_label(label_file):
    # Opens a label file, and passes the object to Object3d object, Read the numpy GT labels
    
    #f = open(label_file)
    #print(label_file)
    label_data = np.load(label_file, allow_pickle=True) 
    #print(f"label_file{label_file} label_data{label_data}")
    #if label_data.any() != None:
...
def objs_to_boxes3d(obj_list):
    ...
    confidence = np.zeros((obj_list.__len__(), 1), dtype=np.float32)
    for k, obj in enumerate(obj_list):
        boxes3d[k, 0:3], boxes3d[k, 3], boxes3d[k, 4], boxes3d[k, 5], boxes3d[k, 6] \
            = obj.pos, obj.h, obj.w, obj.l, obj.ry
        confidence[k] = obj.confidence

    return boxes3d, confidence
