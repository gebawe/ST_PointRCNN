...
def id_type_to_class(id_type):
    id_to_type = {1:'VEHICLE', 2:'Pedestrian', 3:'Cyclist', 4:'Van'}
    if id_type not in id_to_type.keys():
        return -1
    return id_to_type[id_type]

class Object3d(object):
    def __init__(self, line):


        self.kitti_to_kitti = np.array([[1, 0, 0],
                                       [0, 0, 1],
                                       [0, 1, 0]])        
        ...
        self.src = line
        self.cls_id = label[7]
        self.cls_type = id_type_to_class(self.cls_id) #label['label_class']
        ...
        self.alpha = np.arctan2(label[2],label[0])
                
        self.h = float(label[5])
        self.w = float(label[4])
        self.l = float(label[3])
        self.pos_kitti = np.array([float(label[0]), float(label[1]), float(label[2])], dtype=np.float32)
        
        #KITTI Frame
        self.pos = np.dot(self.kitti_to_kitti,self.pos_kitti)
                
        self.ry = float(label[6]) 
        self.score = -1.0
        
        # Pseudo label score hack
        #if 'score' in label.keys():
        if len(label) == 9:
            #print(f"confidence---------{label[8]}")
            self.confidence = label[8]
        else:
            self.confidence = 1
