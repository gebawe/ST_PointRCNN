...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def forward(self,
        ...
        ### hack: confidence weighted loss
        focal_cross_entropy_loss = (modulating_factor * alpha_weight_factor * per_entry_cross_ent)
  
        return focal_cross_entropy_loss * weights * rpn_weights
...
def get_reg_loss(pred_reg, reg_label, label_weights, loc_scope, loc_bin_size, num_head_bin, anchor_size,
    ...
    ### hack: confidence weighted loss
    raw_loss_x_bin = F.cross_entropy(pred_reg[:, x_bin_l: x_bin_r], x_bin_label, reduction='none') # weight label as GT and pseudo
    loss_x_bin = (raw_loss_x_bin * label_weights).mean()

    ### hack: confidence weighted loss
    raw_loss_z_bin = F.cross_entropy(pred_reg[:, z_bin_l: z_bin_r], z_bin_label, reduction='none') # weight label as GT and pseudo
    loss_z_bin = (raw_loss_z_bin * label_weights).mean()
    ...
    if get_xz_fine:
        ...
        ### hack: confidence weighted loss
        raw_loss_x_res = F.smooth_l1_loss((pred_reg[:, x_res_l: x_res_r] * x_bin_onehot).sum(dim=1), x_res_norm_label, reduction='none') # weight label as GT and pseudo
        loss_x_res = (raw_loss_x_res * label_weights).mean()
        
        ### hack: confidence weighted loss
        raw_loss_z_res = F.smooth_l1_loss((pred_reg[:, z_res_l: z_res_r] * z_bin_onehot).sum(dim=1), z_res_norm_label, reduction='none') # weight label as GT and pseudo
        loss_z_res = (raw_loss_z_res * label_weights).mean()
    ...
    if get_y_by_bin:
        ...
        ### hack: confidence weighted loss
        raw_loss_y_bin = F.cross_entropy(pred_reg[:, y_bin_l: y_bin_r], y_bin_label, reduction='none') # weight label as GT and pseudo
        loss_y_bin = (raw_loss_y_bin * label_weights).mean()
        
        ### hack: confidence weighted loss
        raw_loss_y_res = F.smooth_l1_loss((pred_reg[:, y_res_l: y_res_r] * y_bin_onehot).sum(dim=1), y_res_norm_label, reduction='none') # weight label as GT and pseudo
        loss_y_res = (raw_loss_y_res * label_weights).mean()
        ...
        ### hack: confidence weighted loss
        raw_loss_y_offset = F.smooth_l1_loss(pred_reg[:, y_offset_l: y_offset_r].sum(dim=1), y_offset_label, reduction='none') # weight label as GT and pseudo
        loss_y_offset = (raw_loss_y_offset * label_weights).mean()
    ...
    #### hask: confidence weighted loss
    raw_loss_ry_bin = F.cross_entropy(pred_reg[:, ry_bin_l:ry_bin_r], ry_bin_label, reduction='none')  # weight label as GT and pseudo
    loss_ry_bin = (raw_loss_ry_bin * label_weights).mean()
    
    #### hack: confidence weighted loss    
    raw_loss_ry_res = F.smooth_l1_loss((pred_reg[:, ry_res_l: ry_res_r] * ry_bin_onehot).sum(dim=1), ry_res_norm_label, reduction='none') # weight label as GT and pseudo
    loss_ry_res = (raw_loss_ry_res * label_weights).mean()
    ...
    #### hack: confidence weighted loss      
    raw_size_loss = F.smooth_l1_loss(size_res_norm, size_res_norm_label, reduction='none')  # weight label as GT and pseudo
    size_loss = (raw_size_loss * label_weights.repeat(3,1).transpose(1,0)).mean()
