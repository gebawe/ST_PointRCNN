...
kitti_to_kitti = np.array([[1, 0, 0],[0, 0, 1],[0, 1, 0]])
...
def save_argo_format(sample_id, bbox3d, argo_output_dir, scores, lidar_idx_table):
    ...
    fname = lidar_idx_table['%06d'%sample_id]
    argo_output_file = os.path.join(argo_output_dir, 'tracked_object_labels' +fname[2:] + '.json')
...
def save_kittikitt_format(sample_id,  bbox3d, kitti_output_dir, scores, lidar_idx_table):
    corners3d = kitti_utils.boxes3d_to_corners3d(bbox3d)
    print("--------------------")
...
def eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger):
    ...
    rpn_iou_avg = np.zeros(len(thresh_list))

    total_recalled_bbox_list, total_gt_bbox = [0] * 9, 0
    ...
    for data in dataloader:
        ...
        for bs_idx in range(batch_size):
            ...
            ## raw prediction sigmoids (probablity)
            cur_seg_prediction = rpn_scores[bs_idx]
            cur_pts_rect = pts_rect[bs_idx]
            ###print(f"rpn_scores {rpn_scores} cur_seg_result {cur_seg_result}")
            ...
            if not args.test:
                ...
                for ind, FG_THRESH in enumerate(thresh_list):

                    cur_seg_result_th = cur_seg_result > FG_THRESH
                    fg_mask = cur_rpn_cls_label > 0
                    correct = ((cur_seg_result_th == cur_rpn_cls_label) & fg_mask).sum().float()
                    union = fg_mask.sum().float() + (cur_seg_result_th > 0).sum().float() - correct
                
                    rpn_iou = correct / torch.clamp(union, min=1.0)
                    rpn_iou_avg[ind] += rpn_iou.item()
                    ###print(f"cur_seg_result {cur_seg_result} cur_seg_result_th {cur_seg_result_th} rpn_iou {rpn_iou} rpn_iou_avg {rpn_iou_avg}")
            ...
            if args.save_result or args.save_rpn_feature:
                ...
                cur_pred_cls = cur_seg_prediction.cpu().numpy()
                output_file = os.path.join(seg_output_dir, lidar_idx_table['%06d'%cur_sample_id] + '.npy')
                if not args.test:
                    cur_gt_cls = cur_rpn_cls_label.cpu().numpy()
                    output_data = np.concatenate((np.dot(kitti_to_kitti, cur_pts_rect.T).T, cur_gt_cls.reshape(-1, 1), cur_pred_cls.reshape(-1, 1)), axis=1)
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        ###print("Eval mode ----------------------++++ ", list(ret_dict.keys()))
        ...
        if args.save_result:
            ...
            for k in range(batch_size):
                ...
                #save_argo_format(cur_sample_id, roi_boxes3d_np[k], roi_output_dir, roi_scores_raw_np[k], lidar_idx_table)
                #save_argo_format(cur_sample_id, pred_boxes3d_np[k], refine_output_dir, raw_scores_np[k], lidar_idx_table)
        ...
        for k in range(batch_size):
            ...
            ##############################################################################################################
            ### modified 
            keep_idx = iou3d_utils.nms_gpu(boxes_bev_selected, norm_scores_selected, cfg.RCNN.NMS_THRESH).view(-1)
            pred_boxes3d_selected = pred_boxes3d_selected[keep_idx]
            ...
            #image_shape = dataset.get_image_shape(cur_sample_id)
            #save_argo_format(cur_sample_id, pred_boxes3d_selected, final_output_dir, scores_selected, lidar_idx_table)
            save_kittikitt_format(cur_sample_id, pred_boxes3d_selected, final_output_dir, scores_selected, lidar_idx_table)
            ##############################################################################################################
    progress_bar.close()
...
def eval_single_ckpt(root_result_dir):
    ...
    train_eval = False 
    model = PointRCNN(train_eval, num_classes=test_loader.dataset.num_class, use_xyz=True, mode='TEST')
    model.cuda()
...
def create_dataloader(logger):
    ...
    #print('++++++++++++++++++++++++++++++++++++++++++++++++++++ test loader',dir(test_loader))
...
def check_pc_range(xyz):
    ...
    print("xyz[0]",xyz[0], "xyz[1]", xyz[1], "xyz[2]", xyz[2])
    '''
